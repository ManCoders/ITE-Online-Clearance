...
if (isset($_POST['submit'])) {
    ...
    $query = "UPDATE programs SET program_name=?, program_code=? WHERE id=?";
    $stmt = $pdo->prepare($query);
    $stmt->execute([$_POST['major_name'], $_POST['major_code'], $major_id]);

    if ($stmt->rowCount() > 0) {
        header("Location: program.php?success=Major Subject updated successfully!");
    } else {
...
<div class="content">
    <div class="dashboard-container">
        <form action="" method="post">
            <h2>Update Programs Curriculum</h2>
            <input type="hidden" name="major_id" value="<?php echo $_GET['program_id'] ?>">
            <label for="major_name">Program Name : </label>
            <input type="text" name="major_name" value="<?php echo $_GET['program_name'] ?>"
                placeholder="Enter subject Name" /><br><br>
            <label for="major_code">Program Code : </label>
            <input type="text" name="major_code" value="<?php echo $_GET['program_code'] ?>"
                placeholder="Enter subject Code" /><br><br>
